...
def add_to_bag(request, item_id):
    ...
    if item_id in list(bag.keys()):
        ...
        messages.success(request, f'A {product.name} has successfully \
                                     been added to your basket')
    else:
        bag[item_id] = quantity
        messages.success(
            request, f'A {product.name} has successfully been added \
                       to your basket')
...
def adjust_bag(request, item_id):
    ...
    if quantity > 0:
        ...
        bag.pop(item_id)
        messages.success(
            request, f'A {product.name} has successfully been \
